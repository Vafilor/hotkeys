...
function check_status()
    ...
    while not ready and count < 1000 do
        ...
        coroutine.sleep(0.1)
        count = count + 1
        -- TODO - format to 1 decimal point at most.
        -- TODO - add a timer function to format hh:mm:ss whichever is the largest unit
        a:text(string.format("%.1f", recast / 60))
    end
...
windower.register_event('keyboard', function(dik, down, _flags, _blocked)
    -- tilde currently toggles keyboard. Not ideal because maybe you want to write it in chat
    if dik == 41 and not down then
        active = not active
    end
    ...
    if dik == 2 and down then
        return true
    end

    -- TODO need a generic way to trigger actions
    if dik == 2 and not down then -- Keyboard key 1
        windower.send_command('input /ma "' .. spell .. '" <me>')
        a.image:transparency(0.9)
